...
The Vite plugin already owns dev-server lifecycle (`configureServer`), WebSocket setup, and project root resolution — keeping the scanner next to that code avoids circular dependencies and matches how Nuvio bootstraps in dev.
...
When no `nuvio.scanRoots` config is provided, default to:
...
If the same `data-nuvio-id` (or wrapper `id`) appears in more than one file, **fail at index time** with `file:line` diagnostics for each occurrence (see `docs/implPlan.md` Phase 1).
